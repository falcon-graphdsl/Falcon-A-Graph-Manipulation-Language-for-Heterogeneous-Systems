...
if(d2->cpugpu==true&&partcodegen==0){fprintf(FP1,"//graph is partitioned \n");
fprintf(stderr,"XX");
//partforeach=this;
partcodegen=1;
this->expr2->kernel=0;
...
if(1) {
if(d2->gpu==1 /*|| d2->cpugpu==true*/){
if(d2->cpugpu==true)fprintf(FP1,"#pragma omp parallel sections \n{\n#pragma omp section \n{\n");
if(this->feb==0){
...
dir_decl *setvar=NULL;
if(this->stassign!=NULL && this->stassign->rhs->expr_type==FUNCALL){
if(d2->cpugpu==false &&d2->gpugpu==0)
...
fgetpos(FP1,&position);
fprintf(FP1,"%s",tempar);
if(d2->cpugpu==false && d2->gpugpu==0)fprintf(FP1,"#pragma omp parallel for   num_threads(12)\nfor(int id=0;id<%s.npoints;id++)falctfun%d( id,%s);",d2->name,tempfun++,d2->name);
}
