 ...
 if( t1!=t2 )
{
 ...
 if( ((struct struct_hgraph  *)(graph.extra))->minppty[t1].weight==t3&&((struct struct_hgraph  *)(graph.extra))->minppty[t1].weight==((struct struct_hgraph  *)(graph.extra))->minppty[t2].weight )
{
...
mst=hreduxsum0;
printf("\ncorrect if cost < 2^31--> mst cost=%lu\n",mst);
 ...
 for(int i=0;i<hgraph.nedges;i++){ int falcft6=2*i;
 ...
 if( ((struct struct_hgraph  *)(hgraph.extra))->mark[falcft6/2]==true  )
{
 ...
 printf("\ncorrect for int and long int --->mst cost=%lu cnt=%d\n",mst,cnt);
