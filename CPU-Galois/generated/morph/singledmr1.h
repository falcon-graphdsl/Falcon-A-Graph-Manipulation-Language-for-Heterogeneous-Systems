...
void alloc_extra_hgraph(HGraph &hgraph,int flag,int npoints) {
    ...
    ((struct struct_hgraph  *)hgraph.extra)->triangle=(struct node *)malloc(sizeof(struct node) * ((struct struct_hgraph  *)(hgraph.extra))->ntriangle*5) ;
    ((struct struct_hgraph  *)hgraph.extra)->owner=(int *)malloc(sizeof(int) * ((struct struct_hgraph  *)(hgraph.extra))->ntriangle*5) ;
}
