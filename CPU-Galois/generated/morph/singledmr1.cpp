...
((struct struct_hgraph  *)(graph.extra))->triangle[t].obtuse=i; 
 ...
 if( c>cos((30.000000*(22.000000/7.000))/180.000000  ))
return 1;
...
struct node temp=((struct struct_hgraph  *)(graph.extra))->triangle[t];
WL1.push(temp);
} 
...
void   drefine ( struct node  unni ,HGraph & graph ,Pusher0 &current, Pusher1 &collnext ) 
 {
...
((struct struct_hgraph  *)(graph.extra))->triangle[t].isdel=1;
...
collnext.push(((struct struct_hgraph  *)(graph.extra))->triangle[xyx]); 
 ...
 if( !(((struct struct_hgraph  *)(graph.extra))->triangle[t].isdel)&&(((struct struct_hgraph  *)(graph.extra))->triangle[t].isbad)  )
{
 ...
 collnext.push(((struct struct_hgraph  *)(graph.extra))->triangle[t]); 
...
devngraphtriangle=temp1;
//#pragma omp parallel num_threads(12)
 for(int i=0;i<temp1;i++)dinit(i,hgraph);
fprintf(stderr,"dinit over %d\n",badcount);
...
break;
 ...
 int temp7;
temp7=((struct struct_hgraph  *)hgraph.extra)->ntriangle;
printf("%d \n",temp7);
#pragma omp parallel num_threads(12)
 for(int i=0;i<temp7;i++)verify(i,hgraph);
