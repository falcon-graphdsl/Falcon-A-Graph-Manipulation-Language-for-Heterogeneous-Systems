...
fpos_t mainfpos;
extern int FUN_EBLOCK_RET,CPUGPU,MPIFLAG,DUPVAR;
...
extern int gencommcode(FILE*, FILE*, dir_decl*, char**, int);
extern int gencommcodegpu(FILE*, FILE*, dir_decl*, char**, int);
extern int gencommcodecpugpu(FILE*, FILE*, dir_decl*, char**, int);
extern int gencommprefixcode(FILE*, FILE*, dir_decl*, char**, int);
extern int gencommprefixcodegpu(FILE*, FILE*, dir_decl*, char**, int);
extern int gencommprefixcodecpugpu(FILE*, FILE*, dir_decl*, char**, int);
extern int gencommsinglecodegpu(FILE *,FILE *,dir_decl *, char *,int );
extern int gencommsinglestatement(char *,dir_decl *d2,dir_decl *d1,FILE *FP,FILE *FP1);
 extern std::map<char *,statement *>fnames;//foreach call of function in char *with conditional
...
d--;
fprintf(stderr," node %d %s %d\n",expr->expr_type,exptypename[expr->expr_type] ,d);
...
fprintf(FP1,"for(int kk=0;kk<%s.nedges;kk+=FALCK32*1024)RSUM%d<<<FALCK32,1024>>>(%s,kk);\ncudaDeviceSynchronize();\n",d1->name,reduxcnt-1,d1->name);
...
fprintf(FP1,"cudaSetDevice(%d);\n",d1->dev_no);
fprintf(FP1,"for(int kk=0;kk<%s.nedges;kk+=FALCK32*1024)RMUL%d<<<FALCK32,1024>>>(%s,kk);\ncudaDeviceSynchronize();\n",d1->name,reduxcnt-1,d1->name);
fprintf(FP1,"cudaSetDevice(0);\n");
...
NESTED_FOREACH=0;
if(MAINF==1 &&MPIFLAG==1){
fprintf(FP1,"MPI_Finalize();\n");
MAINF=0;
}
...
if(MPIFLAG==1){
fprintf(FP1,"FALCmpiinit(argc,argv);\n");
fprintf(FP1,"sprintf(partitionfile,\"%%s\",argv[2]);\n");
...
if(TARGET<=1)fprintf(FP1,"&(%s),0,1)==0)",arr);
if(TARGET>=2 &&DUPVAR==0)
fprintf(FP1,"&(%s),FALCRANK,MAX_INT-1)==FALCRANK)",arr);
if(TARGET>=2 &&DUPVAR!=0)fprintf(FP1,"&(%s),0,FALCRANK)==0)",arr);
}
}
if(this->f1!=NULL && DUPVAR!=1){this->f1->codeGen(FP1);
}
if(this->f1!=NULL && DUPVAR==1){
statement *tt=this->f1;
int cnt=0;
while (tt!=NULL){
if(tt->sttype==SBLOCK_STMT)cnt++;
if(cnt==0)break;
if(tt->sttype==EBLOCK_STMT)cnt--;
tt=tt->next;
}
tt->codeGen(FP1);
}
if(this->f2!=NULL && DUPVAR!=1){
fprintf(FP1,"else ");
this->f2->codeGen(FP1);
}
}
...
//fprintf(FP1,"/*GPU ASS %d %d %d*/",type,type1,te);
int RANK0=0;
if(TARGET==3 || TARGET==4 || TARGET==5){
tree_expr *expr=this->stassign->lhs;
if(MPIFLAG &&(TARGET==3|| TARGET==4)&& expr && expr->expr_type==STRUCTREF && expr->lhs && expr->lhs->expr_type==STRUCTREF){
expr=expr->lhs;
if(expr->rhs && !strcmp(expr->rhs->name,"points") && expr->rhs->expr_type==ARRREF )RANK0=1;
}
if(RANK0==1){
fprintf(FP1,"\nif(FALCrank==0){\n");
...
fprintf(FP1,"//GPU ASS \n");
 print_gpu_assign_stmt(this,te);
char arr[100];
if(DUPVAR==5)this->stassign->printcode1(arr,1);
fprintf(FP1,"%s \n",arr);
}
}
else {
tree_expr *expr=this->stassign->lhs;
int RANK0=0;
if(MPIFLAG &&TARGET==2&& expr && expr->expr_type==STRUCTREF && expr->lhs && expr->lhs->expr_type==STRUCTREF){
expr=expr->lhs;
if(expr->rhs && !strcmp(expr->rhs->name,"points") && expr->rhs->expr_type==ARRREF )RANK0=1;
}
if(RANK0==1){fprintf(FP1,"if(FALCrank==0){\n");
char arr1[200],arr2[200];
for(int i=0;i<200;i++)arr1[i]=arr2[i]='\0';
if(this->stassign->lhs!=NULL){ this->stassign->lhs->printcode1(this->stassign->lhs,arr1);fprintf(FP1,"%s",arr1);
}
if(this->stassign->rhs!=NULL){ this->stassign->rhs->printcode1(this->stassign->rhs,arr2);fprintf(FP1,"=%s",arr2);
fprintf(FP1,";\n");
}

if(RANK0==1)fprintf(FP1,"}\n");
}
else {
...
if(this->updatecopiescnt>0 &&d2->gpu==0 &&TARGET==2)gencommprefixcode(FP1,FP,d2, this->updatecopies ,this->updatecopiescnt);
if(this->updatecopiescnt>0 &&d2->gpu==1 &&(TARGET==3||TARGET==4))gencommprefixcodegpu(FP1,FP,d2, this->updatecopies ,this->updatecopiescnt);
if(this->updatecopiescnt>0 /*&&d2->gpu==0*/ &&TARGET==5)gencommprefixcodecpugpu(FP1,FP,d2, this->updatecopies ,this->updatecopiescnt);
if(it==0){
this->expr1->lhs->libdtype=POINT_TYPE;
//fprintf(FP1,"ITR=%d\n",it);
if(1) {
if(d2->gpu==1 || TARGET==5){
if(this->feb==0){
for(std::map< char *,statement *>::iterator it1=fnames.begin(); it1!=fnames.end();it1++){
statement *tt=it1->second;
if(!strcmp(it1->first,this->stassign->rhs->name)){
fprintf(FP1, "/*SINGLE FLAG %s %d */\n",this->stassign->rhs->name,tt->stdecl->dirrhs->singleflag);
if( tt->stdecl->dirrhs->singleflag==1){
gencommsinglecodegpu(FP1,FP,d2,"lock",0);
}
}
}
//fprintf(FP1,"/*after*/cudaSetDevice(%d);\nfor(int kk=0;kk<%s.nedges;kk+=%sedgekernelblocks*TPB%d){ \n%s<<<%sedgekernelblocks,TPB%d>>>(",d2->dev_no,d2->name,d2->name,d2->dev_no,this->stassign->rhs->name,d2->name,d2->dev_no);
if(this->stassign!=NULL && this->stassign->rhs->expr_type==FUNCALL){
dir_decl *setvar=NULL;
if(TARGET!=3 && TARGET!=5 && TARGET!=4)fprintf(FP1,"cudaSetDevice(%d);\nfor(int kk=0;kk<%s.npoints;kk+=%spointkernelblocks*TPB%d){ \n%s<<<%spointkernelblocks,TPB%d>>>(",d2->dev_no,d2->name,d2->name,d2->dev_no,this->stassign->rhs->name,d2->name,d2->dev_no);
if(TARGET==4){
if(!strcmp(fnamescond.begin()->first,this->stassign->rhs->name))
fprintf(FP1,"cudaSetDevice(FALCrank);\n \n%s<<<%s.npoints/TPB0+1,TPB0>>>(",this->stassign->rhs->name,d2->name);
else fprintf(FP1,"cudaSetDevice(FALCrank);\n \n%s<<<%s.localpoints/TPB0+1,TPB0>>>(",this->stassign->rhs->name,d2->name);
}
if(TARGET==3){
if(!strcmp(fnamescond.begin()->first,this->stassign->rhs->name))
fprintf(FP1,"cudaSetDevice(%d);\n \n%s<<<%s.npoints/TPB0+1,TPB0>>>(",d2->dev_no,this->stassign->rhs->name,d2->name);
else fprintf(FP1,"cudaSetDevice(%d);\n \n%s<<<%s.localpoints/TPB0+1,TPB0>>>(",d2->dev_no,this->stassign->rhs->name,d2->name);
}
if(TARGET==5){
if(!strcmp(fnamescond.begin()->first,this->stassign->rhs->name))
fprintf(FP1,"cudaSetDevice(%d);\n \n%s<<<%s.gpupart.npoints/TPB0+1,TPB0>>>(",d2->dev_no,this->stassign->rhs->name,d2->name);
else fprintf(FP1,"cudaSetDevice(%d);\n \n%s<<<%s.gpupart.localpoints/TPB0+1,TPB0>>>(",d2->dev_no,this->stassign->rhs->name,d2->name);
}
...
if(ta!=NULL){
if(/*ta->rhs->lhs==d2&&*/ d2->cpugpu==false)
 if((TARGET!=3 && TARGET!=4) && TARGET!=5 )fprintf(FP1,"%s,kk);\n",ta->rhs->name);
if(TARGET==3 ||TARGET==4 || TARGET==5 ) fprintf(FP1,"%s,0);\n",ta->rhs->name);
if(ta->rhs->lhs->libdtype==SET_TYPE )setvar=ta->rhs->lhs;
}
if(TARGET!=3 && TARGET!=5 && TARGET!=4)fprintf(FP1,"}\ncudaDeviceSynchronize();\ncudaSetDevice(0);\n");
if(TARGET==4)fprintf(FP1,"\ncudaDeviceSynchronize();\ncudaSetDevice(FALCrank);\n");
if(TARGET==3 || ( TARGET!=5 && TARGET!=4) )fprintf(FP1,"cudaDeviceSynchronize();\ncudaSetDevice(0);\n");
...
fprintf(FP1, "/*SINGLE FLAG %s %d */\n",this->stassign->rhs->name,tt->stdecl->dirrhs->singleflag);
if( tt->stdecl->dirrhs->singleflag==1){
if(!strcmp(this->stassign->rhs->name,fnamescond.begin()->first))
fprintf(FP1,"#pragma omp parallel for   num_threads(12)\nfor(int i=0;i<%s.localpoints+%s.remotepoints;i++)%s(i,",d2->name,d2->name,this->stassign->rhs->name);
else fprintf(FP1,"#pragma omp parallel for   num_threads(12)\nfor(int i=0;i<%s.localpoints;i++)%s(i,",d2->name,this->stassign->rhs->name);
assign_stmt *ta=this->stassign->rhs->arglist;
ta=ta->next;
while(ta!=NULL && ta->next!=NULL){
char temparr[200];
for(int i=0;i<200;i++)temparr[i]='\0';
if(ta->rhs->lhs!=d2|| d2->cpugpu==false)ta->rhs->printcode1(ta->rhs,temparr);
fprintf(FP1,"%s,",temparr);
if(ta->rhs->lhs->libdtype==SET_TYPE && ta->rhs->lhs->gpu!=1)setvar=ta->rhs->lhs;
ta=ta->next;
}
...
gencommsinglestatement("lock",d2,d2,FP,FP1);
}
}
}
if(!strcmp(this->stassign->rhs->name,fnamescond.begin()->first))
...
if(TARGET==2 || TARGET==5){
fprintf(FP1,"#pragma omp parallel for   num_threads(12)\nfor(int id=0;id<%s.localpoints;id++)falctfun%d( id,%s);",d2->name,tempfun++,d2->name);
...
fprintf(FP1,"#pragma omp parallel for   num_threads(12)\n for(int i=0;i<%s.localpoints;i++){",d2->name);
...
fprintf(FP1, "/*SINGLE FLAG %s %d */\n",this->stassign->rhs->name,tt->stdecl->dirrhs->singleflag);
if( tt->stdecl->dirrhs->singleflag==1){
gencommsinglecodegpu(FP1,FP1,d2,"lock",0);
}
}
}
//fprintf(FP1,"/*after */cudaSetDevice(%d);\nfor(int kk=0;kk<%s.nedges;kk+=%sedgekernelblocks*TPB%d){ \n%s<<<%sedgekernelblocks,TPB%d>>>(",d2->dev_no,d2->name,d2->name,d2->dev_no,this->stassign->rhs->name,d2->name,d2->dev_no);
...
else fprintf(FP1,"/*XX*/for(int falcft%d=0;falcft%d<falcft%d;falcft%d++){\n",unnicnt+2,unnicnt+2,unnicnt,unnicnt+2);
...
if(this->updatecopiescnt>0 && TARGET==2)gencommcode(FP1,FP,d2, this->updatecopies ,this->updatecopiescnt);
if(this->updatecopiescnt>0 && (TARGET==3|| TARGET==4))gencommcodegpu(FP1,FP,d2, this->updatecopies ,this->updatecopiescnt);
if(this->updatecopiescnt>0 && TARGET==5)gencommcodecpugpu(FP1,FP,d2, this->updatecopies ,this->updatecopiescnt);
}
...
dir_decl *t1=this->stdecl->dirrhs;
while(t1!=NULL){
if(cnt>0)fprintf(FP1,","); fprintf(FP1,"%s",t1->name);cnt++;
if(t1->rhs!=NULL){
...
int gf=0,hf=0;
dir_decl *t1=this->stdecl->dirrhs;
while(t1!=NULL){if (t1->gpu==1||DUPVAR==2 )gf=1;
if((t1->gpu || DUPVAR==2) && (t1->ctype==CARR_TYPE && t1->next && t1->next->assign!=NULL)&&gf==0)gf=1;
if((t1->gpu || DUPVAR==2) && !(t1->ctype==CARR_TYPE && t1->next && t1->next->assign!=NULL))gf=2;
if(t1->gpu==0&&(DUPVAR==0))hf=1; if(hf&&gf) break; t1=t1->nextv;}
...
t1=this->stdecl->dirrhs;
while(t1!=NULL){
if (t1->gpu==1){if(cnt>0)fprintf(FP1,","); if(!(t1->ctype==CARR_TYPE &&t1->next&& t1->next->assign!=NULL))t1->printcode(0);cnt++; }
if (DUPVAR && t1->dup_name){if(cnt>0)fprintf(FP1,","); fprintf(FP1,"%s",t1->dup_name);cnt++; }
t1=t1->nextv;
}
fprintf(FP1,";\n");
if(DUPVAR==2)return;
...
//assumes first parameter in function is iterator.
...
if(d2->libdtype==POINT_TYPE &&d2->parent->libdtype!=COLLECTION_TYPE &&TARGET!=3 && TARGET!=5)fprintf(FP1,"%s.npoints",d2->parent->name);
if(d2->libdtype==POINT_TYPE &&d2->parent->libdtype!=COLLECTION_TYPE &&(TARGET==3 || TARGET==5)){

if(!strcmp(this->prev->stdecl->dirrhs->name,fnamescond.begin()->first))
fprintf(FP1,"%s.localpoints+%s.remotepoints",d2->parent->name,d2->parent->name);
else fprintf(FP1,"%s.localpoints",d2->parent->name);
}
}
