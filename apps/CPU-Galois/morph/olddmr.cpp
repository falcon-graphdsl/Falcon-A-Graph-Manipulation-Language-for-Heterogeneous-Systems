...
int   adjacent ( struct node struct node  t1 , t2 ,HGraph & graph ) 
 {
...
struct  cent  getCenter (  t ,HGraph & graph ) 
 {
...
float   radiusSqaure ( struct Point & p1 , t ,HGraph & graph ) 
 {
...
bool  inCircumCircle ( struct Point & P0 , t1 ,HGraph & graph ) 
 {
...
void   drefine ( struct node  t ,HGraph & graph ,Pusher0 &current, Pusher1 &collnext, int   nblocks ,int   size ,int   goal ,unsigned volatile int *   arrayin ,unsigned volatile int *   arrayout ) 
 {
...
conn4= create_coll_device(100);
 ...
 if( !(((struct struct_hgraph  *)(current.extra))->isdel[t])&&(((struct struct_hgraph  *)(current.extra))->isbad[t])  )
{
 ...
 cebot=((struct struct_hgraph  *)(current.extra))->obtuse[t]; 
