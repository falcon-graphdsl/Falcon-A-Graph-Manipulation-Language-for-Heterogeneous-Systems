...
p1.x=graph.points[2*repli0].ipe;
p1.y=graph.points[2*repli0+1].ipe;


 int repli1=((struct struct_hgraph  *)(graph.extra))->triangle[t].nodes[te];
p2.x=graph.points[2*repli1].ipe;
p2.y=graph.points[2*repli1+1].ipe;


 int repli2=((struct struct_hgraph  *)(graph.extra))->triangle[t].nodes[te2];
p3.x=graph.points[2*repli2].ipe;
p3.y=graph.points[2*repli2+1].ipe;
...
int   adjacent (  /**/struct node  t1 ,int  & t2 ,HGraph & graph ) 
 {
 ...
 dimone=(graphnodes[2]==INVALID) ?2 : 3; 

 dimtwo=(graphnodes[2]==INVALID) ?2 : 3; 

 for (int   i =0;i<dimone;i++) {

 for (int   j =0;j<dimtwo;j++) {

 if( t1.nodes[i]==((struct struct_hgraph  *)(graph.extra))->/*4*/triangle[t2].nodes[j]  )
{
...
p1.x=graph.points[2*repli3].ipe;
p1.y=graph.points[2*repli3+1].ipe;


 int repli4=((struct struct_hgraph  *)(graph.extra))->/*4*/triangle[t].nodes[1];
p2.x=graph.points[2*repli4].ipe;
p2.y=graph.points[2*repli4+1].ipe;


 if( dims==3  )
{
 int repli5=((struct struct_hgraph  *)(graph.extra))->/*4*/triangle[t].nodes[2];
p3.x=graph.points[2*repli5].ipe;
p3.y=graph.points[2*repli5+1].ipe;
...
p2.x=graph.points[2*repli6].ipe;
p2.y=graph.points[2*repli6+1].ipe;
...
void   drefine ( struct node  ppp ,HGraph & graph ,Pusher0 &current, Pusher1 &collnext ) 
 {int id=ppp;
...
conn4= create_coll_device(100);
 ...
 t1=((struct struct_hgraph  *)(current.extra))->id[ppp]; 
