...
inline void  adddynamicproperty(tree_expr *t1, enum LIBDATATYPE x,assign_stmt *pt1){
           ...
           if(currsymtab!=NULL && currsymtab->sym!=NULL && dst->lhs!=NULL){
                   tree_typedecl *xu=currsymtab->sym->findtypedefsymbol(dst->lhs->name);// null for basic datatype
                   if(xu!=NULL && xu->list!=NULL){
                          ep->t1=xu;//for compound type such struct datatype xu will no be  null.CHECKING NOT DONE FOR G_P
                          ep->t1->libdatatype=x;//can conflict is struct has libdatatype
           ...
           fprintf(stderr,"ADD NODE PROPERTY %s  %s %d %s \n",tt1->name,t1->lhs->name,dst->lhs->datatype,pt1->rhs->name);
           tt1->ppts=ep;
		...
		struct extra_ppts *ep=malloc(sizeof(struct extra_ppts));
                ...
                tree_typedecl *tt1=new tree_typedecl(); 
                 ...
                 ep->dt1=dst;
                 ep->t1=tt1->t1=(tree_typedecl *)(dst->lhs);
                 ep->t1->libdatatype=x;//added on march23rd;
...
void addgraphproperty(tree_expr *t1, enum LIBDATATYPE x, assign_stmt *pt1){
            ...
            tree_typedecl *tt1=new tree_typedecl();
            tt1->libdatatype=x;
            tt1->d1=currsymtab->findsymbol(t1->lhs->name);//tt1->d1 will be graph
            ep->var2=NULL;
            ep->val2=-1;
           tree_decl_stmt *dst=(tree_decl_stmt *)t1->rhs->params;
           ep->t1=tt1->t1=(tree_typedecl *)(dst->lhs);
           ...
           tt1->name=malloc(sizeof(char)*100);
           strcpy(tt1->name,libdtypenames[tt1->libdatatype]);
           dir_decl *pptsize=dst->dirrhs;//second argument of addProperty
           dir_decl *var=new dir_decl(); 
           var->name=malloc(sizeof(char)*100); 
           strcpy(var->name,pt1->rhs->name);
           ep->var1=var;
           ...
           fprintf(stderr,"ADD GRAPH PROPERTY %s  %s %d %s %s \n",tt1->name,t1->lhs->name,dst->lhs->datatype,dst->lhs->name,pt1->rhs->name);

           tt1->ppts=ep;
          if(ep->var2!=NULL)fprintf(stderr, "\n XXXXXXXXXXXXX %s   \n",ep->var2->name);
          if(ep->var1!=NULL)fprintf(stderr, "\n VAR1XXXXXXXXXXXXX %s   \n",ep->var1->name);
          if(ep->var3!=NULL)fprintf(stderr, "\n VAR3XXXXXXXXXXXXX %s   \n",ep->var3->name);
          symtableentry *old=NULL;
          currsymtab->addsymbol((dir_decl *)var,(tree_typedecl *)tt1);
          tt1->d2=new dir_decl();
...
//fprintf(FP1,"//if else iF STMT %d\n",temp3->sttype);
//fprintf(stderr,"//IF ELSE IF STMT\n");
t4->f1=t2;
t4->f2=t1;
t4->sttype=IF_STMT;
statement *tt=temp;
...
return;
		 ...
		 if(t2->sttype==SBLOCK_STMT){
			...
			t2->prev->prev->next=t4;
			 ...
			 if(temp3->prev->sttype==SBLOCK_STMT)fprintf(stderr,"HELLO CATCHED HERE");
